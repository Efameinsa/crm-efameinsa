// Espera a que el despliegue del commit 2e8c33c (0205: el Word trae las tres
...
for (let i = 0; i < 60; i++) {
  ...
  const d = (r.deployments ?? []).find((x) => (x.meta?.githubCommitSha ?? "").startsWith("2e8c33c"));
  const ahora = new Date().toLocaleTimeString("es-PE", { timeZone: "America/Lima", hour: "2-digit", minute: "2-digit" });
  if (!d) console.log(`${ahora} aún no aparece el despliegue de 2e8c33c (último: ${r.deployments?.[0]?.meta?.githubCommitSha?.slice(0,7)} ${r.deployments?.[0]?.state})`);
  else {
    console.log(`${ahora} ${d.state} ${d.url}`);
    if (d.state === "READY") { console.log("LISTO: la tanda 2 del 10-09 en producción."); process.exit(0); }
