// Espera a que el despliegue del commit 1f1c53b (0205: el Word trae las tres
...
for (let i = 0; i < 60; i++) {
  ...
  const d = (r.deployments ?? []).find((x) => (x.meta?.githubCommitSha ?? "").startsWith("1f1c53b"));
  const ahora = new Date().toLocaleTimeString("es-PE", { timeZone: "America/Lima", hour: "2-digit", minute: "2-digit" });
  if (!d) console.log(`${ahora} aún no aparece el despliegue de 1f1c53b (último: ${r.deployments?.[0]?.meta?.githubCommitSha?.slice(0,7)} ${r.deployments?.[0]?.state})`);
  else {
    console.log(`${ahora} ${d.state} ${d.url}`);
    if (d.state === "READY") { console.log("LISTO: la historia sin montos para postventa en producción."); process.exit(0); }
