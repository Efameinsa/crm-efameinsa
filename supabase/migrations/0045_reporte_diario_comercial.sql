-- ============================================================
-- CRM EFAMEINSA · Migración 0045 · Reporte diario del comercial
-- ============================================================
-- El ing. Carlos lo marcó como "vital" (reunión 20-08): hoy cada comercial
-- arma a mano una agenda diaria en Excel, la exporta a PDF y la manda por
-- correo al cierre del día. El ejemplo que compartió —"C5 AGENDA 03-08-26
-- Katerine Tello.pdf"— tiene cinco secciones y sus totales.
--
-- La idea es que deje de copiarse a mano: "ellos lo copian y pegan de su CRM,
-- acá ya no, yo voy ingresando, voy generando mis presupuestos, se supone que
-- al final del día simplemente genero un reporte en PDF, lo bajo y lo envío".
--
-- Esta función devuelve el DETALLE del día (no solo los conteos que ya da
-- supervision_diaria, que es la vista de gerencia sobre todos los comerciales).
--
-- LAS ACTIVIDADES COMPLEMENTARIAS son las tareas personales del día ya
-- completadas (tabla tareas_agenda): capacitación, visita a campo, reunión
-- comercial. Carlos: "alguna actividad que no esté relacionada a seguimientos,
-- presupuestos o cotizaciones, ponemos complementariamente". No hace falta una
-- tabla nueva — es exactamente para lo que se creó tareas_agenda.
--
-- AUTORIZACIÓN: el propio comercial (para bajar su reporte) o backoffice
-- (para revisarlo). Un comercial no puede pedir el de otro.

create or replace function reporte_diario_comercial(p_comercial uuid, p_fecha date default null)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  join cuentas cu on cu.id = o.cuenta_id
  where o.comercial_id = p_comercial and o.origen = 'crm' and v.fecha_venta = v_fecha;
...
revoke all on function reporte_diario_comercial(uuid, date) from public;
grant execute on function reporte_diario_comercial(uuid, date) to authenticated;

comment on function reporte_diario_comercial(uuid, date) is
  'Detalle de la jornada de un comercial para el PDF de cierre del día (docs: formato en Downloads/reunion20agosto). Lo puede pedir el propio comercial o backoffice.';
