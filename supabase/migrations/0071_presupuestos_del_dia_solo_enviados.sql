-- ============================================================
-- CRM EFAMEINSA · Migración 0071 · «Presupuestos del día» = los que salieron
-- ============================================================
-- Pedido de gerencia el 24-08, con la foto del reporte a la vista: «que muestre
-- solo cotizaciones del período ejecutadas o enviadas, no borradores».
--
-- El bloque «2. PRESUPUESTOS DEL DÍA» decía «TOTAL: 4 (enviados: 2)» y listaba
-- dos con estado "Borrador". Un borrador no es trabajo entregado: desde la
-- migración 0064 ni siquiera tiene número, y puede no enviarse nunca. Contarlo
-- infla el reporte que gerencia usa para medir el día.
--
-- Y CAMBIA LA FECHA POR LA QUE SE AGRUPA, que es lo que hace que el número
-- signifique algo: antes por `created_at` —el día en que se EMPEZÓ a armar— y
-- ahora por `enviada_at`, el día en que el cliente lo recibió. Una cotización
-- empezada el viernes y enviada el lunes es trabajo del lunes.
--
-- DE PASO, UN FILTRO QUE VACIABA LAS VENTAS. El bloque de ventas exigía
-- `o.origen = 'crm'`, y ninguna oportunidad de un comercial tiene ese origen:
-- las ventas del día salían siempre en cero. Es el mismo filtro que había
-- vaciado el Kanban y «Mi día», y que ya se quitó de esas dos pantallas.
--
-- Esta función es la de la migración 0045 con esos dos cambios y nada más: se
-- generó a partir de aquel archivo para no reescribirla de memoria.
