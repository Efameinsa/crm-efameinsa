-- ============================================================
-- CRM EFAMEINSA · Migración 0059 · Derivaciones por comercial en la supervisión
-- ============================================================
-- Los tres lados pidieron el mismo número el 24-08, primer día de uso real:
--
--   · CENTRAL: hasta ahora reportaba a mano al ingeniero cuántos prospectos
--     derivó y cuántas cotizaciones salieron. Ese conteo lo hacía copiando al
--     Excel; el Excel ya no se usa, así que se quedó sin forma de armarlo.
--   · GERENCIA: «¿cuántos prospectos le hemos otorgado a C4, C5?» — la tabla
--     de carga por comercial que esperaba ver en su panel.
--   · EL COMERCIAL: «¿cuántos te han entregado y cuántos en el día?». Sin ese
--     numerador, "3 cotizaciones" no dice si trabajó bien o mal.
--
-- Es un solo dato y estaba a un join de distancia: leads.asignado_a +
-- asignado_at. Se agrega a supervision_diaria() en vez de crear una función
-- nueva, para que las tres pantallas lean de la misma fuente y no se
...
begin
  ...
  -- Leads que CENTRAL le derivó ese día. Es el numerador que le faltaba a
  -- todo lo demás: sin saber cuántos le entregaron, "3 cotizaciones" no
  -- significa nada. Lo pidieron los tres lados el 24-08 — Central para su
  -- consolidado, gerencia para la carga por comercial, y el comercial para
  -- ver cuántos le tocaron hoy.
  left join lateral (
    select count(*) as n
    from leads l
