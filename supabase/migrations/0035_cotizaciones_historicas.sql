-- ============================================================
-- CRM EFAMEINSA · Migración 0035 · Presupuestos historicos en el conteo
-- ============================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'cot_historicas_periodo', (select count(distinct v.referencia_historica)
                                from ventas v join oportunidades o on o.id = v.oportunidad_id
                                where v.origen = 'historico_excel' and v.referencia_historica is not null
  ...
  left join lateral (
    ...
    select count(*) as sin_monto,
           (select count(distinct v3.referencia_historica)
              from ventas v3 join oportunidades o3 on o3.id = v3.oportunidad_id
              where o3.comercial_id = p.id and v3.origen = 'historico_excel'
                and v3.referencia_historica is not null
                and v3.fecha_venta between p_desde and p_hasta
